...
- **app.rs** - Application state (`App` struct), modes (Normal/Search/Filter/Help), conversation navigation
- **data/** - Data structures and schema
  - **conversation.rs** - `Conversation`, `Message`, `Role`, `Metadata` types
  - **loader.rs** - `Schema` struct for custom JSONL formats
- **filter_expr.rs** - Filter expression parser (AND/OR/NOT, parentheses, comparisons, tag/note filters)
- **search_index.rs** - Search optimizations: trigram index for O(1) substring search, sorted field index for O(log n) range queries
- **streaming_loader.rs** - Incremental JSONL loading with glob pattern support
...
2. `StreamingLoader` incrementally reads JSONL files, creates `Vec<Conversation>` in batches
3. `App::new_empty()` initializes state, conversations added via `add_conversations()`
...
- **Filtering**: Supports AND/OR/NOT expressions with parentheses, e.g., `(score>90 AND x<50) OR is_flag=true`
- **Custom Schema**: Non-standard JSONL files use `-u`/`-a`/`-s` flags to map fields to user/assistant/system messages
- **Glob Patterns**: Load multiple files with patterns like `**/*.jsonl`
- **On-demand Indexing**: Press `I` to build trigram index for instant substring search
